...
conteo_grado = (
    conteo_grado.merge(diccionario, how="inner", on="item")
...
conteo_grado["resp"] = (
    conteo_grado["resp"]
    .astype("category")
    .cat.reorder_categories(DESC_RESP, ordered=True)
)
...
for proceso in sel_proceso:
    ...
    nom_criterios = [f"{i}<br>{j}" for i, j in zip(html_criterios, texto_criterios)]
